...
- **Stakeholder report**: `python contracts/report_generator.py -r <report.json>` writes `validation_reports/stakeholder__<contract_id>__<run_timestamp>.json` by default (pipeline-discoverable name); add `--pdf` to emit a matching `.pdf` beside it. Use `-o` / `--pdf path` to override. Add `--fail-on-registry-gap` in CI if every failing field must appear in the registry.
